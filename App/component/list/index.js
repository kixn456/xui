...
const ARROW_RIGHT = require('../../image/arrow-right.png');
...
export default class List extends Component {
    ...
    renderBtn() {
        ...
        this.props.btnConfig.map((item, index) => {
            let backgroundColor = item.backgroundColor ? item.backgroundColor : utils.theme.mainColor;
            let width = item.width ? item.width : 80;
            let fontSize = item.fontSize ? item.fontSize : 16;
            let color = item.color ? item.color : 'white';
            let onPress = item.onPress ? item.onPress : () => {};
            btnList.push(
                <TouchableOpacity
                    key = {index}
